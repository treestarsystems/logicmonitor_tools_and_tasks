...
export class BackupServiceGeneral {
  ...
  async backupGeneralGet(
    ...
    try {
      ...
      if (resultList.status === 'failure') {
        ...
        const errorMessage = this.utilsService.defaultErrorHandlerString(resultList.message);
  ...
  async retrieveBackupsAll(company: string, response: Response): Promise<void> {
    ...
    try {
      ...
      const backupsListGeneral = await this.storageServiceMongoDb.find(this.backupGeneralModel, {
        company: company,
        type: { $not: new RegExp('datasource', 'i') },
      });
  ...
  private async processPayloadItems(
    ...
    for (const pli of payloadItems) {
      ...
      try {
        ...
        const errMsg = this.utilsService.defaultErrorHandlerString(err);
        progressTracking.failure.push(`Failure: ${backupNameParsed} - ${errMsg}`);
      }
    }
  }
}
